...
- **端末間同期**: 進捗タブで同期キーを設定すると、回答記録が D1 (`s-assess-quiz-db`) に自動保存され全端末で共有される。起動時にpull+全件push、回答ごとに即push。追記専用+主キー重複排除なので競合しない。API は `functions/api/answers.js` (認証: `X-Sync-Key` ヘッダー = Pages secret `SYNC_KEY`)。同期キーの値はローカルの `.sync-key.local` (git管理外) にある。キーを変える場合は `wrangler pages secret put SYNC_KEY --project-name s-assess-quiz` と各端末の進捗タブを更新
- D1バインディングは `wrangler.jsonc` で定義 (**注意: 親ディレクトリに別プロジェクトの wrangler.jsonc があるため、デプロイは必ずこのプロジェクト直下で実行する。toml だと親のjsoncに負けて設定が読まれない**)
